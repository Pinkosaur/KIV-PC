#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "mp_int.h"
#include "fact.h"

/* ----------------- Factorial using precomputed table + product_range ----------------- */

const FactEntry fact_table[] = {
    {0, "1"},
    {1, "1"},
    {2, "2"},
    {5, "120"},
    {10, "3628800"},
    {50, "30414093201713378043612608166064768844377641568960512000000000000"},
    {75, "24809140811395398091946477116594033660926243886570122837795894512655842677572867409443815424000000000000000000"},
    {100, "93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000"},
    {200, ""},
    {256, ""},
    /* Beyond 256! the strings are too long for ANSI C */
    {0, NULL} /* sentinel */
};

/* Helper: create mp_int representing small unsigned value */
static int mp_set_uint(mp_int *dst, unsigned int v) {
    if (!dst) return FAILURE;
    mp_free(dst);
    mp_init(dst);
    if (v == 0) {
        dst->sign = 0;
        dst->length = 0;
        return SUCCESS;
    }
    if (mp_reserve(dst, 1) != SUCCESS) return FAILURE;
    dst->digits[0] = v;
    dst->length = 1;
    dst->sign = +1;
    return SUCCESS;
}

/* mp_fact: compute factorial of integer 'a' and store in r.
   Expects 'a' to be non-negative and reasonably small (fits into uint32_t).
   Uses precomputed factorials and product_range for remaining range.
*/
int mp_fact(mp_int *r, const mp_int *a)
{
    mp_int i, tmp, acc;
    size_t best_idx;
    int found;

    if (!r || !a) return FAILURE;

    if (a->sign < 0) {
        fprintf(stderr, "Input of factorial must not be negative!\n");
        return FAILURE;
    }

    /* 0! = 1, 1! = 1 */
    if (a->sign == 0 ||
        (a->length == 1 && a->digits[0] == 1U)) {
        mp_free(r);
        mp_init(r);
        return mp_set_uint(r, 1U);
    }

    /* Find largest precomputed factorial <= a */
    best_idx = 0;
    found = 0;
    {
        size_t k;
        mp_int n_mp;
        mp_init(&n_mp);

        for (k = 0; fact_table[k].fact_str != NULL; ++k) {
            mp_free(&n_mp);
            mp_init(&n_mp);
            mp_from_str_dec(&n_mp, fact_table[k].fact_str);
            if (mp_cmp_abs(a, &n_mp) >= 0) {
                best_idx = k;
                found = 1;
            }
        }
        mp_free(&n_mp);
    }

    /* acc = starting factorial */
    mp_init(&acc);
    if (found) {
        if (mp_from_str_dec(&acc, fact_table[best_idx].fact_str) != SUCCESS) {
            mp_free(&acc);
            return FAILURE;
        }
    } else {
        mp_set_uint(&acc, 1U);
        best_idx = 0;
    }

    /* i = fact_table[best_idx].n + 1 */
    mp_init(&i);
    mp_set_uint(&i, fact_table[best_idx].n + 1U);

    mp_init(&tmp);

    /* for i <= a: acc *= i */
    while (mp_cmp_abs(&i, a) <= 0) {

        if (mp_fits_uint(&i)) {
            /* fast path */
            if (mp_mul_small(&acc, i.digits[0]) != SUCCESS)
                goto fail;
        } else {
            /* general mp_int multiply */
            if (mp_mul(&tmp, &acc, &i) != SUCCESS)
                goto fail;
            mp_free(&acc);
            mp_init(&acc);
            mp_copy(&acc, &tmp);
        }

        if (mp_inc(&i) != SUCCESS)
            goto fail;
    }

    mp_free(r);
    mp_init(r);
    mp_copy(r, &acc);

    mp_free(&i);
    mp_free(&tmp);
    mp_free(&acc);
    return SUCCESS;

fail:
    mp_free(&i);
    mp_free(&tmp);
    mp_free(&acc);
    return FAILURE;
}
